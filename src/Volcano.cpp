...
public:
    ...
    void initIMGUI(int height, int width)
    {
        ...
        myFonts.push_back(io.Fonts->AddFontFromFileTTF((myResourcePath + std::string("fonts/Cousine-Regular.ttf")).c_str(), 16.0f, &config));
        myFonts.push_back(io.Fonts->AddFontFromFileTTF((myResourcePath + std::string("fonts/DroidSans.ttf")).c_str(), 16.0f, &config));
        myFonts.push_back(io.Fonts->AddFontFromFileTTF((myResourcePath + std::string("fonts/Karla-Regular.ttf")).c_str(), 16.0f, &config));
        myFonts.push_back(io.Fonts->AddFontFromFileTTF((myResourcePath + std::string("fonts/ProggyClean.ttf")).c_str(), 16.0f, &config));
        myFonts.push_back(io.Fonts->AddFontFromFileTTF((myResourcePath + std::string("fonts/ProggyTiny.ttf")).c_str(), 16.0f, &config));
        myFonts.push_back(io.Fonts->AddFontFromFileTTF((myResourcePath + std::string("fonts/Roboto-Medium.ttf")).c_str(), 16.0f, &config));
        ...
        ImGui_ImplVulkanH_CreateWindowDataCommandBuffers(myPhysicalDevice, myDevice, myQueueFamilyIndex, &myWindowData, myAllocator->GetAllocationCallbacks());
        ImGui_ImplVulkanH_CreateWindowDataSwapChainAndFramebuffer(myPhysicalDevice, myDevice, &myWindowData, myAllocator->GetAllocationCallbacks(), width, height, true);
        ...
        : myResourcePath(resourcePath)
    {
        ...
        {
             int x, y, n;
             unsigned char* data = stbi_load((myResourcePath + std::string("images/Vulkan_500px_Dec16.png")).c_str(), &x, &y, &n, STBI_rgb_alpha);
         
             assert(data != nullptr);
         
             createDeviceLocalImage2D(data, x, y, 4, VK_IMAGE_USAGE_SAMPLED_BIT, myImage, myImageMemory);
...
private:
    ...
    void createDebugCallback()
    {
        ...
        CHECK_VK(vkCreateDebugReportCallbackEXT(myInstance, &debugCallbackInfo, nullptr, &myDebugCallback));
    }
    ...
    {
        auto vsCode = readSPIRVFile((myResourcePath + std::string("shaders/spir-v/vert.spv")).c_str());
        ...
        auto fsCode = readSPIRVFile((myResourcePath + std::string("shaders/spir-v/frag.spv")).c_str());
        ...
        VkRect2D scissor = {};
        scissor.offset = { 0, 0 };
        scissor.extent = { static_cast<uint32_t>(myWindowData.Width), static_cast<uint32_t>(myWindowData.Height) };
        ...
        CHECK_VK(vkCreateGraphicsPipelines(myDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &myGraphicsPipeline));
    ...
    void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags flags, VkBuffer& outBuffer, VmaAllocation& outBufferMemory)
    {
        ...
        CHECK_VK(vmaCreateBuffer(myAllocator, &bufferInfo, &allocInfo, &outBuffer, &outBufferMemory, nullptr));
    }
    ...
    void createImage2D(uint32_t width, uint32_t height, uint32_t pixelSizeBytes, VkImageUsageFlags usage, VkMemoryPropertyFlags memoryFlags, VkImage& outImage, VmaAllocation& outImageMemory)
    {
        ...
        CHECK_VK(vmaCreateImage(myAllocator, &imageInfo, &allocInfo, &outImage, &outImageMemory, &outAllocInfo));
    }
    ...
    void checkFlipOrPresentResult(VkResult result)
    {
        ...
        if (result == VK_ERROR_OUT_OF_DATE_KHR)
            ImGui_ImplVulkanH_CreateWindowDataSwapChainAndFramebuffer(myPhysicalDevice, myDevice, &myWindowData, myAllocator->GetAllocationCallbacks(), myWindowData.Width, myWindowData.Height);
    ...
    void submitFrame(uint32_t frameIndex)
    {
        ...
        checkFlipOrPresentResult(vkAcquireNextImageKHR(myDevice, myWindowData.Swapchain, UINT64_MAX, imageAquiredSemaphore, VK_NULL_HANDLE, &myWindowData.FrameIndex));
        ...
        /*
        {
            ...
            checkFlipOrPresentResult(vkAcquireNextImage2KHR(myDevice, &nextImageInfo, &myWindowData.FrameIndex));
        }
         */
        ...
        // draw animated image
        {
            ...
            vkCmdBindDescriptorSets(newFrame->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, myPipelineLayout, 0, 1, &myDescriptorSet, 0, nullptr);
            vkCmdBindPipeline(newFrame->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, myGraphicsPipeline);
            ...
            vkCmdDrawIndexed(newFrame->CommandBuffer, static_cast<uint32_t>(sizeof_array(ourIndices)), 1, 0, 0, 0);
        }
    ...
    void cleanup()
    {
        ...
        ImGui_ImplVulkanH_DestroyWindowData(myInstance, myDevice, &myWindowData, myAllocator->GetAllocationCallbacks());
...
const Vertex VulkanApplication::ourVertices[] =
{
